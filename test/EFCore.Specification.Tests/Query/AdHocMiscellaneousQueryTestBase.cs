    ...
    : NonSharedModelTestBase(fixture), IClassFixture<NonSharedFixture>
{
    ...
    [ConditionalFact]
    public virtual async Task Left_join_with_projected_nominal_type_should_be_null_when_no_match()
    {
        var contextFactory = await InitializeNonSharedTest<Context22568>(seed: c => c.SeedAsync());

        using var context = contextFactory.CreateDbContext();

        var dbPosts = from p in context.Set<Context22568.Post>()
                      select new Context22568.Post
                      {
                          PostId = p.PostId,
                          BlogId = p.BlogId,
                          Content = p.Content
        ...
        var blogWithPost = result.Single(b => b.Url == "blog1");
        Assert.NotNull(blogWithPost.Post);
        Assert.Equal("content1", blogWithPost.Post.Content);

        var blogWithoutPost = result.Single(b => b.Url == "blog2");
        Assert.Null(blogWithoutPost.Post);
    }
    ...
    protected class Context22568(DbContextOptions options) : DbContext(options)
    {
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Blog>().Ignore(b => b.Post);
        ...
        public Task SeedAsync()
        {
            ...
            AddRange(
                new Post { PostId = 1, BlogId = 1, Content = "content1" });
            return SaveChangesAsync();
        }
